...
storing blobs in sequence of monolithic data files indexed by a sorted kv index.
...
func newIndex(root string, indexConf jsonconfig.Obj) (sorted.KeyValue, error) {
	...
	return sorted.NewKeyValue(jsonconfig.Obj(map[string]interface{}{
		"type": defaultIndexType, "file": filepath.Join(root, defaultIndexFile),
	}))
}
