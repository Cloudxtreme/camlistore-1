...
// Package compress registers the "compress" blobserver storage type
// which stores all blobs and metadata with zlib, snappy or xz compression,
// wrapped storage targets (e.g. localdisk, s3, remote, google).
//
// A compress storage target is configured with two other storage targets:
// one to hold compressed blobs, and one to hold metadata about
// the compressed blobs. On start-up, all the metadata blobs are read
...
type storage struct {
	...
	// blobs holds compressed versions of all plaintext blobs.
	blobs blobserver.Storage

	// meta holds metadata mapping between the names of plaintext
	// blobs and their after-compression name, as well as their
	// compression method. Each blob in meta contains 1 or more blob
	// description. All new insertions generate both a new
	// compressed blob in 'blobs' and one single-meta blob in
	...
	/*
		...
		// smallMeta tracks a heap of meta blobs, sorted by their compressed size
