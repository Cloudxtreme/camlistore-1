...
var punchHole func(file *os.File, offset int64, size uint32) error
...
func (s *storage) delete(br blob.Ref) error {
	...
	if b[0] != byte('[') || b[k-1] != byte(']') {
		return fmt.Errorf("delete: cannot find header surroundings, found %v", b)
	}
	b = b[1 : k-1] // "sha1-xxxxxxxxxxxxxxxxxx"
	off += 1
